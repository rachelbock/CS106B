 ...
 *
 * A testing harness and driver for the Huffman encod:g
 * assignment.
 */
...
//	for (ext_char key : referenceTable) {
 ...
 * --------------------------------------------------------
 * Test code for getFrequencyTable	If you are fail:g these
...
void testBuildEncodingTree() {
	beginTest("buildEncodingTree tests (just kidding, this doesn't do anything right now)");

	endTest("buildEncodingTree tests (Totall passed! Doing nothing!)");
}
 ...
 * correctly.	 These tests look for things like the follow:g:
 *
...
void testEncoding() {
	...
	for (string file : files) {
		...
		assertCondition(input.is_open(), ("Cannot open file test/encodeDecode/" + file + " for read:g!"));
...
/* Function: openFile
 * --------------------------------------------------------
 * Prompts the user for the name of a file to open, open:g
 ...
 */
void runCompress() {
	/* Open a file for read:g. */
	ifbstream infile;
	openFile(infile, "File to compress: ");

	/* Open a file for writ:g. */
	ofbstream outfile;
